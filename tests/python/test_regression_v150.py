...
class TestCovarianceNuggetAtZeroLag:
    ...
    def test_kriging_with_nugget_produces_different_results(self):
        ...
        result_no = ordinary_kriging(
            prop=prop, grid=grid, radiuses=(3, 3, 2), max_neighbours=8, cov_model=cov_no_nugget
        )
        result_with = ordinary_kriging(
            prop=prop, grid=grid, radiuses=(3, 3, 2), max_neighbours=8, cov_model=cov_with_nugget
        )
...
class TestOKKrigingVarianceSign:
    ...
    def test_ok_single_informed_cell(self):
        ...
        cov = CovarianceModel(
            type=covariance.spherical, ranges=(3.0, 3.0, 1.0), sill=1.0, nugget=0.0
        )

        result = ordinary_kriging(
            prop=prop, grid=grid, radiuses=(3, 3, 1), max_neighbours=4, cov_model=cov
        )
...
class TestCokrigingCrossCovarianceRatio:
    ...
    def test_cokriging_with_positive_correlation(self):
        ...
        cov = CovarianceModel(
            type=covariance.exponential, ranges=(5.0, 5.0, 2.0), sill=1.0, nugget=0.1
        )
    ...
    def test_cokriging_with_negative_correlation(self):
        ...
        cov = CovarianceModel(
            type=covariance.spherical, ranges=(5.0, 5.0, 2.0), sill=1.0, nugget=0.1
        )
