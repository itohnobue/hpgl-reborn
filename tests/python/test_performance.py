...
def _benchmark(name, elapsed, hard_floor=120.0):
    ...
    if baseline is None:
        ...
        print(
            f"  [BENCHMARK] {name}: {elapsed:.3f}s (baseline: {baseline:.3f}s, {factor:.1f}x, floor: {hard_floor}s) [{status}]"
        )
    if not result:
        print(
            f"  WARNING: {name} exceeds performance threshold! "
...
class TestPerformance:
    ...
    def test_covariance_type_performance(self):
        ...
        for cov_type, cov_name in [
            ...
            cov_model = CovarianceModel(type=cov_type, ranges=(5.0, 5.0, 3.0), sill=1.0, nugget=0.1)
