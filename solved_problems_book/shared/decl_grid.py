...
class Grid:
	def __init__(self, x_min, y_min, dx, dy, l1, l2):
		...
		self.data1 = []
		self.data2 = []
		self.x_cell = np.zeros((dx+1), dtype=float)
		self.y_cell = np.zeros((dy+1), dtype=float)
		for i in range(dx+1):
	...
	def get_center_points(self):
		self.x_center = np.zeros((self.dx), dtype=float)
		self.y_center = np.zeros((self.dy), dtype=float)
	...
	def get_weights_cell(self):
		w = np.zeros((len(self.data1)), dtype=float)
		n = 0
		n_1 = 0
		for i in range(len(self.data1)):
	...
	def calc_e_h(self, x_center, y_center, c):
		e_h = 0.0
		for i in range(len(self.data1)):
			e_h = e_h + (np.sqrt((x_center-self.data1[i])**2+(y_center-self.data2[i])**2))**(-c)
		return e_h

	def calc_h(self, x_center, y_center, j, c):
		h = (np.sqrt((x_center-self.data1[j])**2+(y_center-self.data2[j])**2))**(-c)
		return h
	...
	def hgt_calc(self, x0, y0, n_p):
		h = np.zeros((len(self.data1)), dtype=float)
		j = 0
		for i in range(len(self.data1)):
	...
	def get_weights_idw(self, x_center, y_center, c):
		w1 = np.zeros((len(self.data1)), dtype=float)
...
def get_rect(array_x, array_y):
	min_max = np.zeros((4), dtype=float)
