...
x = 10
y = 4
data_dict = load_gslib_file("samples.txt")
...
data_points_variable = data_dict["Variable"]
data_points_easting = data_dict["Easting"]
data_points_northing = data_dict["Northing"]
# ----------------------------------------------------
...
quantiles = np.zeros((2), dtype=float)
# 1: 0.025 quantile
quantiles[0] = (1 - p) / 2
# 2: 0.975 quantile
quantiles[1] = (1 + p) / 2
...
# ----------------------------------------------------
#	(2) Transform the data to be standard normal and recalculate the 95% probability interval.
# ----------------------------------------------------


# Transforming data to standard normal form
for i in range(x):
...
cov_h = np.zeros((x, x), dtype=float)
...
var_cov = cov_h.mean()
print("Variance of standardized data in presence of correlation is: ", var_cov)
...
print("Variance of data in presence of correlation is: ", var_cov_orig)
