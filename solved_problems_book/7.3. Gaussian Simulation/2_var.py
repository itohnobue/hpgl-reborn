...
for i in range(i_max):
	for j in range(j_max):
		for k in range(k_max):
			arithmetic_mean = get_sum_cell_value(array_grid, x_coord, y_coord, z_coord, i, j, k, data_dict[value], type='a')
...
var_arithmetic = np.var(prop_ijk.compress((prop_ijk != -99).flat))
...
for i in range(i_max):
	for j in range(j_max):
		for k in range(k_max):
			geometric_mean = get_sum_cell_value(array_grid, x_coord, y_coord, z_coord, i, j, k, data_dict[value], type='g')
			if (geometric_mean > 0):
				prop_geometric[i, j, k] = geometric_mean
			else:
				prop_geometric[i, j, k] = -99
var_geometric = np.var(prop_geometric.compress((prop_geometric != -99).flat))
...
for i in range(i_max):
	for j in range(j_max):
		for k in range(k_max):
			harmonic_mean = get_sum_cell_value(array_grid, x_coord, y_coord, z_coord, i, j, k, data_dict[value], type='h')
			if (harmonic_mean > 0):
				prop_harmonic[i, j, k] = harmonic_mean
			else:
				prop_harmonic[i, j, k] = -99
var_harmonic = np.var(prop_harmonic.compress((prop_harmonic != -99).flat))
print("Harmonic variance:", var_harmonic)

var_initial = np.var(data_dict[value])
print("Point scale variance:", var_initial)

initial_data_arithmetic = np.copy(prop_ijk)

# Transform initial data with cdf_transform function
transformed_data = np.copy(prop_ijk)
...
var_back_krigged_transformed_data = np.var(back_krigged_transformed_data[0])
...
plt.figure()
plt.hist(sgs_harmonic_data[0].flat, bins=20)
plt.title("Simulated field from harmonic averaging")

# Histogram of simulated field from arithmetic averaging
plt.figure()
plt.hist(sgs_arithmetic_data[0].flat, bins=20)
plt.title("Simulated field from arithmetic averaging")

# Histogram of kriged field from harmonic averaging
plt.figure()
plt.hist(krigged_harmonic_data[0].flat, bins=20)
plt.title("Kriged field from harmonic averaging")

# Histogram of kriged field from arithmetic averaging
plt.figure()
plt.hist(krigged_arithmetic_data[0].flat, bins=20)
plt.title("Kriged field from arithmetic averaging")
...
array_threshold_initial = data_dict[value].compress((data_dict[value] > threshold_value).flat)
print((float(len(array_threshold_initial)) / (len(data_dict[value]))) * 100, "% of the data lies above the threshold")
...
plt.figure()
plt.hist(data_dict[value], bins=20, cumulative=True)
plt.axvline(threshold_value, linewidth=2, color='k')
plt.title("Cumulative point-scale data")

plt.figure()
plt.hist(krigged_arithmetic_data[0].flat, bins=20, cumulative=True)
plt.axvline(threshold_value, linewidth=2, color='k')
plt.title("Cumulative kriged field from arithmetic averaging")

plt.figure()
plt.hist(sgs_arithmetic_data[0].flat, bins=20, cumulative=True)
plt.axvline(threshold_value, linewidth=2, color='k')
plt.title("Cumulative simulated field from arithmetic averaging")
