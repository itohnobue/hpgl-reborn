...
quantiles = np.zeros(2, dtype=np.float32)
# 1: 0.025 quantile
quantiles[0] = (1-p)/2
# 2: 0.975 quantile
quantiles[1] = (1+p)/2
...
probs_trans = np.zeros(2, dtype=np.float32)
...
# ----------------------------------------------------
#	(3) Consider now that the data are spatially dependent, and this dependece is characterized by a correlation function specified as C(h) = exp(-0.2h) where h is calculated as the distance between the samples.
# ----------------------------------------------------
...
cov_h = np.zeros((x, x), dtype=np.float32)
...
probs_cov = np.zeros(2, dtype=np.float32)
