	...
	*/	
	...
	public:		
		...
		{			
			...
			int ry = rs[1];
			int rz = rs[2];
			// Shared radius-magnitude guard (I2-24 / F-M3): a catchable
			// hpgl_exception replaces the previous abort() — SIGABRT is
			// uncatchable by Python/ctypes.  Runs BEFORE any signed-int
			// diameter arithmetic (rx*2+1) so a radius near INT_MAX cannot
			// overflow UB-first.  Returns the validated (2r+1)³ volume.
			const size_t size = validate_covariance_radiuses_or_throw(rx, ry, rz,
				"precalculated_covariances_t::init");
			dx = - rx;
			dy = - ry;
			dz = - rz;
			sx = rx * 2 + 1;
